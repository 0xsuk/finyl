...
void rekordbox_anlz_t::_read() {
    m__unnamed0 = m__io->read_bytes(4);
    if (!(_unnamed0() == std::string("\x50\x4D\x41\x49", 4))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\x50\x4D\x41\x49", 4), _unnamed0(), _io(), std::string("/seq/0"));
    }
...
void rekordbox_anlz_t::phrase_up_down_t::_clean_up() {
}
...
void rekordbox_anlz_t::phrase_verse_bridge_t::_clean_up() {
}
...
void rekordbox_anlz_t::song_structure_tag_t::_read() {
    ...
    const int l_entries = len_entries();
...
void rekordbox_anlz_t::cue_extended_entry_t::_read() {
    m__unnamed0 = m__io->read_bytes(4);
    if (!(_unnamed0() == std::string("\x50\x43\x50\x32", 4))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\x50\x43\x50\x32", 4), _unnamed0(), _io(), std::string("/types/cue_extended_entry/seq/0"));
    }
    m_len_header = m__io->read_u4be();
...
void rekordbox_anlz_t::song_structure_entry_t::_read() {
    ...
    case rekordbox_anlz_t::PHRASE_STYLE_UP_DOWN: {
        m_phrase_id = new phrase_up_down_t(m__io, this, m__root);
        break;
    }
    case rekordbox_anlz_t::PHRASE_STYLE_VERSE_BRIDGE: {
...
void rekordbox_anlz_t::song_structure_entry_t::_clean_up() {
    if (m_phrase_id) {
        delete m_phrase_id; m_phrase_id = 0;
    }
}
...
void rekordbox_anlz_t::cue_entry_t::_read() {
    m__unnamed0 = m__io->read_bytes(4);
    if (!(_unnamed0() == std::string("\x50\x43\x50\x54", 4))) {
        throw kaitai::validation_not_equal_error<std::string>(std::string("\x50\x43\x50\x54", 4), _unnamed0(), _io(), std::string("/types/cue_entry/seq/0"));
    }
    m_len_header = m__io->read_u4be();
    m_len_entry = m__io->read_u4be();
    m_hot_cue = m__io->read_u4be();
